...
export function updateTopScores(playerId, playerName, playerScore, playerClicks, topScores) {
    let isTopScoresChanged = false;
    ...
    if (playerScore < topScores[last].score ||
        ...
        let foundPlayerIndex = topScores.findIndex(element => element.uid === playerId);
        if (foundPlayerIndex === -1) {
            //New player will add to TopScores and replace the last player in TopScores
            topScores.splice(last, 1, { uid: playerId, name: playerName, score: playerScore, clicks: playerClicks });
            ...
            if (playerScore < topScores[foundPlayerIndex].score ||
                ...
                topScores.splice(foundPlayerIndex, 1, { uid: playerId, name: playerName, score: playerScore, clicks: playerClicks });
                isTopScoresChanged = true;//Enable Update to Firebase Button
            }
        }
...
export function updateBestScore(level,player, playerScore, playerClicks) {
    let isBestScoresChanged = false;
    if (level === 16){
        if (playerScore < player.bestEasyScore ||
            (playerScore === player.bestEasyScore && playerClicks < player.bestEasyClicks)) {
            ...
            if (playerScore < player.bestHardScore ||
                (playerScore === player.bestHardScore && playerClicks < player.bestHardClicks)) {
                //Update best score of the player in playerData
